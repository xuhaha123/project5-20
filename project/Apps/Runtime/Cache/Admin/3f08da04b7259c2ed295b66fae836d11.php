...
<!-- <!--Action boxes-->
  ...
  <div class="container-fluid">
    <div class="quick-actions_homepage">
      <ul class="quick-actions">
        <li class="bg_lb"> <a href=""> <i class="icon-signal"></i> <span class="label label-important">10</span>用户组列表 </a> </li>
      </ul>
    </div>
  </div>
  ...
  <form action="<?php echo U('Admin/Quan/group');?>" method="get" id="mid">
  ...
  <thead>
    <tr role="row">
      <th class="ui-state-default" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" style="width: 83px;" aria-sort="ascending" aria-label="Rendering engine: activate to sort column descending">
       ...
       <th class="ui-state-default" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" style="width: 80px;" aria-label="Browser: activate to sort column ascending">
        <div class="DataTables_sort_wrapper">组名
      ...
      <!-- <th class="ui-state-default" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" style="width: 200px;" aria-label="Browser: activate to sort column ascending">
        <div class="DataTables_sort_wrapper">组用户
            <span class="DataTables_sort_icon css_right ui-icon ui-icon-carat-2-n-s"></span></div>
      </th> -->
      <th class="ui-state-default" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" style="width:372px;" aria-label="Platform(s): activate to sort column ascending">
        <div class="DataTables_sort_wrapper">权限名称
            <span class="DataTables_sort_icon css_right ui-icon ui-icon-carat-2-n-s"></span></div>
      </th>
      <th class="ui-state-default" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" style="width: 72px;" aria-label="Engine version: activate to sort column ascending">
        <div class="DataTables_sort_wrapper">状态
            <span class="DataTables_sort_icon css_right ui-icon ui-icon-carat-2-n-s"></span>
        </div>
      </th>

      <th class="ui-state-default" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" style="width: 170px;" aria-label="Engine version: activate to sort column ascending">
...
<tbody role="alert" aria-live="polite" aria-relevant="all">
  <?php if(is_array($groups)): $i = 0; $__LIST__ = $groups;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="gradeA odd">
          ...
          <td style="width:100px;"><?php echo ($vo["rules"]); ?></td>
          <!-- <td class="center " style="text-align:center">
          <button class="bts"><?php switch($vo["status"]): case "1": ?>开启<?php break;?>
                                    <?php case "0": ?>关闭<?php break; endswitch;?>
          </button></td> -->
          <td class="center " style="text-align:center"><button class="bts"><?php echo ($vo["status"]); ?></button></td>
          <td class="center ">
            <a href="<?php echo U('Admin/Quan/update1',array('id'=>$vo['id']));?>">
  ...
  $(function(){
        $('.bts').bind("click",function(){
            ...
            var $this = $(this);
            if(display == '开启'){
                $.post('<?php echo U("Admin/Quan/group_ajax");?>',
                    {id:id,status:0},
                    function(data){
                        if(data)
                            $this.html('关闭');
        ...
        $('.bt-de').click(function(){
            var id = $(this).parents('tr').find('td:first').html();
            // alert(id);
            var tr = $(this);
            $.get('<?php echo U("Admin/Quan/group_delete");?>',{id:id},function(data){
                if(data)
