...
<div class="typrography">
  ...
  <div id="main-p">
     ...
     <style>
        *{margin:0;padding:0;}
        dialog{display:block;width:500px;height:375px;border:1px solid gray;padding:10px;margin:0 auto;}
    </style>
      <div class="col-md-6 dal">
        <form action="<?php echo U('Home/User/insert');?>" method="get">
          <h3 style="color:black;margin-top:35px;">用户评论</h3>         

        <dialog>
        <br>
        书名:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" class="text" value="<?php echo ($value["book"]); ?>" onfocus="this.value = '<?php echo ($value["book"]); ?>';" onblur="if (this.value == '') {this.value = '<?php echo ($value["book"]); ?>';}" name="book" readonly>
        <textarea onfocus="if(this.value == '') this.value='<?php echo ($comms['context']); ?>';" onblur="if(this.value == '') this.value='<?php echo ($comms['context']); ?>';" name="context" readonly><?php echo ($value["context"]); ?></textarea><br>
        <input type="submit" value="显示回复评论内容" disabled="true"><br><br>
        <textarea onfocus="if(this.value == '') this.value='<?php echo ($comms['reply']); ?>';" onblur="if(this.value == '<?php echo ($comms['reply']); ?>') this.value='<?php echo ($comms['reply']); ?>';"name="reply" disabled="true"><?php echo ($value["reply"]); ?></textarea>
